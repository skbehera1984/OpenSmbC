...
#define HASH_ALGORITHM_SHA_512              0x0001
#define SMB2_SHA_512_SIZE                   (512 / 8)
...
typedef struct _smb2_neg_ctx_hdr
{
  ...
  uint16_t DataLength;
  uint32_t Reserved;
} __attribute__ (( __packed__ )) smb2_neg_ctx_hdr;

typedef struct smb2_preauth_integ_ctx
{
  smb2_neg_ctx_hdr hdr;
  uint16_t HashAlgorithmCount;
  uint16_t SaltLength;
  uint16_t HashAlgorithms; // it should be an array, but for now there is only one
  uint8_t  Salt[32]; // it is a variable buffer, but most windows use a 32 byte salt
} __attribute__ (( __packed__ )) smb2_preauth_integ_context;

/* Encryption Ciphers SMB3.11 */
#define SMB2_ENC_CIPHER_AES_128_CCM 0x0001
#define SMB2_ENC_CIPHER_AES_128_GCM 0x0002

typedef struct smb2_enc_cap_ctx
{
  smb2_neg_ctx_hdr hdr;
  uint16_t CipherCount;
  uint16_t Ciphers[2]; // its an array, but for now there are only two of these
} __attribute__ (( __packed__ )) smb2_enc_cap_context;

#define SMB2_NEG_PREAUTH       0x01
#define SMB2_NEG_ENC_CAP       0x10
...
struct smb2_negotiate_request
{
  ...
  uint8_t neg_ctx_flags;
  smb2_preauth_integ_context preauth_ctx;
  smb2_enc_cap_context       enc_ctx;
};
...
class smb2_security_descriptor
{
public:
  smb2_security_descriptor();
  ~smb2_security_descriptor();
