                              ...
                              std::string&   error)
{
  ...
  {
    error = "smb2_decode_file_full_ea_info:Failed to allocate ea name";
    return false;
  }
  ...
  {
    error = "smb2_decode_file_full_ea_info:Failed to allocate ea value";
    free(info->ea_name);
    return false;
  }
  ...
  memcpy(info->ea_value, vec->buf + (8 + info->ea_name_length), info->ea_value_length);
                               ...
                               std::string&   error)
{
  ...
  do
  {
    ...
    if (offset >= vec->len)
    {
      error = "smb2_decode_file_extended_info:Malformed query reply";
      return -1;
    }
    ...
    if (!smb2_decode_file_full_ea_info(smb2, &full_ea_info, &tmp_vec, error))
    {
      return -1;
    }
    ...
    if ( full_ea_info.next_entry_offset != 0 )
    {
      ...
      if (next_node == NULL)
      {
        ...
        error = "smb2_decode_file_extended_info:Failed to allocate smb2_file_extended_info";
        return -1;
      }
...
int
smb2_encode_file_extended_info(Smb2ContextPtr smb2,
                               struct smb2_file_extended_info *info,
                               const int      count,
                               uint8_t        *buffer,
                               uint32_t       *buffer_len,
                               string&        error)
{
  ...
  {
    error = "smb2_encode_file_extended_info:Buffer not allocated for file ea info";
    return -1;
  }
  ...
  while(entries < count)
  {
    struct smb2_file_full_ea_info *full_ea_info = (struct smb2_file_full_ea_info *)(buffer+offset);
    ...
    // copy to buffer
    offset += sizeof(struct smb2_file_full_ea_info) - (2*sizeof(uint8_t*));
                                  ...
                                  string&        error)
{
  ...
  {
    error = "smb2_decode_file_full_stream_info:Failed to allocate memory for stream name";
    return false;
  }
                             ...
                             string&        error)
{
  ...
  do
  {
    ...
    if (offset >= vec->len)
    {
      ...
      error = "smb2_decode_file_stream_info:Malformed query reply";
      return -1;
    }
    ...
    if (!smb2_decode_file_full_stream_info(smb2, &full_stream_info, &tmp_vec, error))
    {
      smb2FreeFileStreamInfo(info);
      return -1;
    }
    ...
    if ( full_stream_info.next_entry_offset != 0 )
    {
      ...
      {
        smb2FreeFileStreamInfo(info);
        error = "smb2_decode_file_stream_info:Failed to allocate smb2_file_stream_info";
        return -1;
      }
